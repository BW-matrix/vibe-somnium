...
| `CampaignHandoff` | cross-scene transition | explicit materialization boundary that derives a later fixture only from a fixture-hash-bound committed scene packet, owner-specific memory, pressure, and audit metadata | use for bounded consecutive-scene studies before a persistent campaign store exists | never forward quarantined state, raw prompts, foreign-owner memory, candidate material, or prose as world truth | `two-scene-continuity-study-v0.3.md` |
| `fixture_sha256` | source-execution binding | canonical content hash of the exact fixture used to create a runtime trace | require an exact match before a retained Scene 1 trace may be reused to materialize Scene 2 | matching `trace_id` and `scene_id` alone is insufficient | `two-scene-continuity-study-v0.3.md` |
| `CharacterPresentationProfile` | proposed public identity view | stable reader-facing Character data such as canonical name, pronouns, honorifics, and naming register | use to preserve literary presentation across scenes without promoting prior prose into world truth | draft only; not private cognition, not a full Character sheet, and not yet executable | `two-scene-continuity-study-v0.3.md` |
| `reserved_protocol_ids` | cross-scene identity guard | all prior-scene model/runtime identities plus Kernel-generated ScenePacket and MemoryDelta identities that a later scene may reference but must not claim again | use to extend replay prevention across a materialized scene boundary | keep separate from ids newly claimed by the current runtime state; an incomplete set fails continuity review | `two-scene-continuity-study-v0.3.md` |
| `campaign_chain_sha256` | continuity audit hash | hash over the committed first packet hash, materialized second fixture hash, and committed second packet hash | use to identify one exact two-scene evidence chain | not a digital signature or proof of literary quality | `two-scene-continuity-study-v0.3.md` |
