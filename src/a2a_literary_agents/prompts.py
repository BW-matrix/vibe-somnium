...
AGENT_INSTRUCTIONS = {
    ...
    "character": (
        ...
        "Choose exactly one action_type and keep every externally observable act in "
        "public_surface within that type. Do not bundle speech with a physical action, "
        "or a physical action with speech, in one proposal; defer the second act to a "
        "later Character decision. "
    ...
    "world": (
        ...
        "For a non-speech EventProposal, spoken_line_records must be []; quoted wording "
        "embedded in a mismatched public_surface is not committed speech. If deterministic "
        "repair feedback reports mixed speech and physical action, commit only the act "
        "owned by action_type and use a bounded partial outcome when needed. "
    ...
    "authority": (
        ...
        "For EventProposal, verify that action_type covers every externally observable act "
        "in public_surface. A proposal that bundles physical action and speech under one "
        "action_type must receive repair_required with a schema_only repair; it must not be "
        "allowed as an atomic Character action. "
